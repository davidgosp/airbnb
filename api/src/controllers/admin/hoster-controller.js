...
const Hoster = sequelizeDb.Hoster
const Op = sequelizeDb.Sequelize.Op

exports.create = async (req, res, next) => {
  try {
    const data = await Hoster.create(req.body)
...
exports.findOne = async (req, res, next) => {
  try {
    ...
    const data = await Hoster.findByPk(id)
...
exports.update = async (req, res, next) => {
  try {
    const id = req.params.id
    const [numberRowsAffected] = await Hoster.update(req.body, { where: { id } })
...
exports.delete = async (req, res, next) => {
  try {
    const id = req.params.id
    const numberRowsAffected = await Hoster.destroy({ where: { id } })
