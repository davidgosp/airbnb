(async () => {
  ...
  try {
    ...
    let neighburhoods = data.reduce((acc, element) => {
      ...
      acc[element.neighburhood]['total'] = acc[element.neighburhood]['total'] ? acc[element.neighburhood]['total'] + parseFloat(element.reviewScoresRating) : parseFloat(element.reviewScoresRating)
    ...
    await fs.writeFile('../data/review-score-by-neighburhood.json', JSON.stringify(neighburhoodsSortedByAverage, null, 2))
